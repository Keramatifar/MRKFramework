<?php
    ...
    date_default_timezone_set(@date_default_timezone_get()); // crud code not depends on timezone, but this can fix some warnings
    ...
    class Xcrud
    {
        ...
        protected $upload_folder_def = '../../uploads/javazm';
        ...
        public static function get_requested_instance()
        {
            ...
            if (isset($_SESSION['xcrud_session'][$inst_name]['key']) && $_SESSION['xcrud_session'][$inst_name]['key'] == $key)
            {
                ...
                self::error('<strong>کلید امنیتی باطل شده است، لطفا مجددا وارد سیستم شوید</strong><br />
                </strong>');
        }
        ...
        public function language($lang = 'fa')
        {
            $this->language = $lang;
            return $this;
        }
        ...
        {
            $subselect_before = $this->subselect_before;
            $dbh = Xcrud_db::get_instance($this->connection);
            foreach ($this->columns as $key => $col)
            {
                ...
                {
                    // 
                    //if($this->nameByColumn)

                    $dbhNAME = Xcrud_config::$dbname; 
                    $tableNAME = $this->table; 

                    //$result = mysqli_fetch_assoc(mysqli_query(xcrud_db::get_instance(),
                    $q = "SELECT COLUMN_COMMENT cc
                    FROM information_schema.COLUMNS
                    WHERE TABLE_SCHEMA = '$dbhNAME' AND TABLE_NAME = '$tableNAME' AND COLUMN_NAME = '$col[field]'";
                    $qq = $dbh->query("$q");

                    $res = $dbh->row();
                     $fielKey = $tableNAME . '.' . $col['field'] ;
//array_push($this->fieldComments, ["$fielKey"=> $res['cc']]);

                    /*      print_r($res);
                    echo '<hr />';
                    echo  $res['cc'];
                    echo '<hr />';*/
                    //echo $this->html_safe($this->_humanize($res['cc']));
                    if(strlen($res['cc']) > 0)
                        $this->columns_names[$key] = $this->html_safe($this->_humanize($res['cc'])) ;
                    else
                    {
        ...
        protected function _set_field_names()
        {
            ...
            foreach ($this->fields as $key => $field)
            {
                ...
                // $this->fields_names[$key] =$this->fieldComments[$key];
                {
                     $dbhNAME = Xcrud_config::$dbname; 
                    $tableNAME = $this->table; 
                    ...
                    $q = "SELECT COLUMN_COMMENT cc
                    FROM information_schema.COLUMNS
                    WHERE TABLE_SCHEMA = '$dbhNAME' AND TABLE_NAME = '$tableNAME' AND COLUMN_NAME = '$field[field]'";
                    $qq = $dbh->query("$q");

                    $res = $dbh->row();
                    $this->fields_names[$key] = $res['cc'];
                }
                 ...
                 /*   $dbhNAME = Xcrud_config::$dbname; 
                    ...
                    //echo $this->html_safe($this->_humanize($res['cc']));
                    if(strlen($res['cc']) > 0)
                        $this->fields_names[$key] = $this->html_safe($this->_humanize($res['cc'])) ;
                    else
                    {
                    ...
                    '&#42;' : '');*/
                  //  $this->fields_names[$key] = $this->html_safe($this->_humanize($['field'])) . (isset($this->validation_required[$key]) ?
                  //  '&#42;' : '');
        ...
        protected function create_bool($name, $value = '', $tag = array())
        {
            ...
            return  /*$this->theme_config('checkbox_container')*/ /*. $this->open_tag('label')*/  $this->single_tag($tag,
                $this->theme_config('bool_field'), $this->field_attr[$name], true) /*. $this->close_tag('label')*/ /*. $this->close_tag('span')*/;
        }
        ...
        protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
        {
            ...
            if ($this->is_title)
            {
                ...
                if ($to_show && !$this->start_minimized)
                    $attr['style'] = 'display:none;';
                if ($to_show)
                $attr['class'] = 'xcrud-main-tab YekanWeb';
